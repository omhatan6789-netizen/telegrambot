...
async def begin_hide_game(
    ...
    # حفظ مؤقت الاختباء حتى نستطيع إلغاءه
    # إذا اختار جميع اللاعبين قبل انتهاء الـ60 ثانية
    game["hide_task"] = asyncio.create_task(
        hide_phase_timeout(
            context,
            chat.id
        )
...
async def hide_number_callback(
    ...
    if len(game["hidden"]) == len(game["players"]):
        ...
        # إلغاء مؤقت الـ60 ثانية
        hide_task = game.get("hide_task")
...
async def finish_hiding(
    ...
    # إلغاء مؤقت الاختباء إذا كان ما زال يعمل
    hide_task = game.get("hide_task")

    if hide_task and hide_task is not asyncio.current_task():
        hide_task.cancel()

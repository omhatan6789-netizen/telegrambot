...
async def command_guard(
    ...
    if command in moderation_commands:
        return
    # ==================================================
    # تشخيص وقت فحص الصلاحية
    # ==================================================
    debug_total_start = time.perf_counter()
    # ==================================================
    # فحص صلاحية المستخدم الخاصة
    # ==================================================
    ...
    print(
        f"⏱️ command_guard | "
        f"check_user_permission = "
        f"{permission_time:.4f}s | "
        f"command = {command}"
    )
    # ==================================================
    # ممنوع
    ...
    if special_permission is False:
        print(
            f"⏱️ command_guard | "
            f"TOTAL = "
            f"{time.perf_counter() - debug_total_start:.4f}s"
        )
    ...
    # فحص قفل الأمر
    # ==================================================
    command_permission_start = time.perf_counter()
    try:
        allowed, required = (
            await asyncio.to_thread(
                check_command_permission,
        ...
        return
    command_permission_time = (
        time.perf_counter()
        - command_permission_start
    )
    print(
        f"⏱️ command_guard | "
        f"check_command_permission = "
        f"{command_permission_time:.4f}s | "
        f"command = {command}"
    )
    # ==================================================
    # الأمر مقفول
